...
import com.arpablue.jsontree.JSONtreePrinter;
 ...
 */
public class JSONtree extends JSONtreePrinter {
    protected int mStrPos = 0;
    public JSONtree() {
        super();
    }
    public static Object getProperty(String json, String path){
 ...
 current JSONtree object.
     ...
     */
    protected boolean parse(String target){
        if( target == null){
            return false;
        }
        this.clear();
        ...
        if( target.charAt( mStrPos - 1 ) == '[' ){
           parseArrayString( target, this);
        }
        convertFromString(target,this);
        return true;
    }
    /**
     * It parseObject a string that contain an array.
     ...
     */
    protected void parseArrayString(String target, JSONtree tree){
        if( target == null ){
            return;
        }
    ...
    /**
     * It get the data to fill the JSONtree from a string in JSON  format.
     * @param target It is the string that contain the JSON format.
     * @param tree
     * @param pos It is the current position in the string.
     */
    protected void convertFromString(String target, JSONtree tree){
        if( target == null ){
            return;
        }
        if( mStrPos >= target.length() - 1 ){
    ...
    /**
     * It show the structure of a JSONtree in the console.
